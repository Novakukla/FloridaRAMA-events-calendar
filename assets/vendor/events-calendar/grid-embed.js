(function () {
  ...
  // WordPress serves the merged feed. GitHub remains a temporary fallback.
  var DATA_BASE = window.FR_EVENTS_DATA_BASE || 'https://novakukla.github.io/FloridaRAMA-events-calendar/';

  function getDataUrl() {
    return window.FR_EVENTS_DATA_URL || '';
  }
  ...
  // Load shared.css immediately (required for layout)
  loadCSS(BASE + 'shared.css');

  // Use the theme font stack instead of pulling an extra Google Font.
  ...
  style.textContent =
    ...
    '}' +
    '.fr-pagination { display: flex; justify-content: center; align-items: center; gap: 16px; padding: 10px 0 20px; font-family: inherit; }' +
  ...
  function renderPage() {
    ...
    for (var p = 0; p < pageItems.length; p++) {
      ...
      if (isMobile) {
        ...
        // Local vendored fix: card clicks always open detail; tickets stay inside detail/back face.
        (function (c, itm) {
  ...
  // ── Boot ──
  async function loadJsonArray(path, throwOnError) {
    try {
  ...
  async function init() {
    ...
    try {
      ...
      if (dataUrl) {
        try {
          merged = await loadJsonArray(dataUrl, true);
        } catch (feedError) {
          console.warn('WordPress event feed unavailable; using GitHub fallback.');
        }
      }

      if (!merged.length) {
        var results = await Promise.all([
          loadJsonArray(DATA_BASE + 'events.json'),
          loadJsonArray(DATA_BASE + 'manual-events.json')
        ]);
